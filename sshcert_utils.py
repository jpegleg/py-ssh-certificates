...
def deflate_long(num, add_sign_padding=True):
    ...
    while (num != 0) and (num != -1):
        bytestring = pack(">I", num & xffffffff) + bytestring
        num >>= 32
    # strip off leading zeros, FFs
    for i in enumerate(bytestring):
        if (num == 0) and (i[1] != deflate_zero):
            break
        if (num == -1) and (i[1] != deflate_ff):
            break
    else:
    ...
    if add_sign_padding:
        if (num == 0) and (byte_ord(bytestring[0]) >= 0x80):
            bytestring = zero_byte + bytestring
        if (num == -1) and (byte_ord(bytestring[0]) < 0x80):
...
def decode_string(source_data: bytes) -> tuple:
    size = unpack('>I', source_data[:4])[0]+4
    return source_data[4:size], source_data[size:]

def decode_int(source_data: bytes) -> tuple:
    return int(unpack('>I', source_data[:4])[0]), source_data[4:]

def decode_int64(source_data: bytes) -> tuple:
    return int(unpack('>Q', source_data[:8])[0]), source_data[8:]

def decode_mpint(source_data: bytes) -> tuple:
    data_str, remainder = decode_string(source_data)
    return inflate_long(data_str), remainder

def decode_list(source_data: bytes) -> tuple:
    layer_one, remainder = decode_string(source_data)
    
    lst = []
    while len(layer_one) > 0:
        elem, layer_one = decode_string(layer_one)
        lst.append(decode_string(elem))
        
    return lst, remainder

def decode_dsa_signature(source_data: bytes) -> tuple:
    signature = []
    layer_one, remainder = decode_string(source_data)
    ...
    signature['s'], encoded_sig = decode_mpint(encoded_sig)
